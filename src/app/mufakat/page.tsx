...
export default async function MufakatPage() {
  ...
  const categories = ['Infrastruktur', 'Pendidikan', 'Kesehatan', 'Lingkungan', 'Ekonomi'];
        ...
        <div style={{ marginBottom: '40px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <div>
              ...
              <p style={{ color: 'var(--text-secondary)' }}>Sila 4: Menjunjung tinggi Kerakyatan melalui musyawarah digital transparan.</p>
            </div>
          </div>
        </div>
      </ScrollReveal>
      ...
      <div className="responsive-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {safePetitions.map((petition, idx) => {
              const target = 15000;
              const progress = Math.min((petition.votes_count / target) * 100, 100);
              const category = categories[idx % categories.length];
              return (
                <ScrollReveal key={petition.id} delay={idx * 0.06}>
                  <div className="glass-card" style={{ padding: '24px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                      <div>
                        <span className="badge badge-sila4" style={{ marginBottom: '8px', display: 'inline-block' }}>{category}</span>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8rem', color: 'var(--text-muted)', background: 'var(--bg-hover)', padding: '4px 8px', borderRadius: '4px' }}>
                        <Target size={14} /> {Math.max(1, 7 - idx)} Hari lagi
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                      ...
                      <div style={{ width: '100%', height: '8px', background: 'var(--border-subtle)', borderRadius: '4px', overflow: 'hidden' }}>
                        <div style={{ width: `${progress}%`, height: '100%', background: 'var(--sila4)', borderRadius: '4px', transition: 'width 1s ease' }}></div>
                      </div>
                    </div>

...
export default async function NusantaraPage() {
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      {/* Hero — immersive dark section with parallax */}
      <div style={{
            ...
            <div className="glass-card" style={{ padding: '24px' }}>
              {safeDict.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
                  {safeDict.map((item, idx) => (
                    <ScrollReveal key={item.id} delay={idx * 0.04} direction="up">
                      <div style={{ padding: '16px', background: 'var(--bg-base)', borderRadius: '12px', border: '1px solid var(--border-subtle)' }}>
